mod javac {
    use crate::tools::JavacConfig;
    use jabu_config::model::JavaConfig;
    #[test]
    fn test_args() {
        let expected = vec![
            "file.java",
    ...
    fn test_with_classpath() {
        ...
        let classpath = vec!["lib/file.jar".to_string(), "../project/src/".to_string()];
...
mod java {
    ...
    fn java_only_class() {
        ...
        let expected = vec!["me.user.app.App"];
        let java_tool = JavaToolConfig::new(
    ...
    fn java_jar_with_classpath() {
        let expected = vec![
            "-cp",
            "./lib/dependency.jar:./lib/other_dependency.jar",
            "-jar",
            "./file.jar",
        ];
        let classpath = vec![
        ...
        let java_tool = JavaToolConfig::new(
            JavaExecTarget::Jar("./file.jar".to_string()),
            classpath,
            Vec::new(),
        );
        assert_eq!(expected, java_tool.into_args());
    ...
    fn java_class_with_classpath() {
        ...
        let java_tool = JavaToolConfig::new(
            JavaExecTarget::MainClass("me.user.app.App".to_string()),
            classpath,
            Vec::new(),
        );
        assert_eq!(expected, java_tool.into_args());
...
mod jar {
    ...
    fn compression_with_manifest() {
        let expected = vec![
            ...
            "./target.jar".to_string(),
            "--manifest".to_string(),
            "./target/bin/MANIFEST.MF".to_string(),
            "-C".to_string(),
            "target/classes".to_string(),
            ".".to_string(),
        ];
        let mut config =
            JarToolConfig::new("./target.jar".to_string(), "target/classes".to_string());
...
mod javadoc {
    use crate::tools::{JavaVisibilityLevel, JavadocToolConfig};
    use jabu_config::model::JavaConfig;
    ...
    fn simple_javadoc_args() {
        ...
        let input = JavadocToolConfig::new(
            vec![
                ...
                "src/main/registry/Person.java".to_string(),
            ],
