...
use crate::tools::{JavaExecTarget, JavaHome, JavaToolConfig};
...
impl JabuTask for Run {
    ...
    fn execute(
        ...
        let main_class: &str = if let Some(Some(main_class)) =
            parsed_args.get_option_value("main-class")
        {
            main_class
        } else if let Some(main_class) = jabu_config.manifest.get("Main-Class") {
            main_class.as_str()
        } else {
        ...
        let mut classpath = vec![jabu_config
            .fs_schema
            .target_classes()
            .to_string_lossy()
            .to_string()];
        let jars: Vec<String> = crate::utils::walkdir_find(
            ...
            &[
                crate::utils::FSNodeType::File,
                crate::utils::FSNodeType::SymLink,
            ],
        )
        .iter()
