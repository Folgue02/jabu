...
use jabu_config::{model::JabuProject, fsutils::libs};
...
impl JabuTask for ListDepsTask {
    fn execute(&self, _: Vec<String>, _: Option<ParsedArguments>, jabu_config: &JabuProject, _: &JavaHome) -> TaskResult {
        let dep_names: Vec<String> = libs(None, jabu_config).iter()
...
impl ListDepsTask {
    fn list_dependencies(jabu_config: &JabuProject) {
