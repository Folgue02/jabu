...
impl From<jaburepo::error::RepositoryOperationError> for TaskError {
    fn from(value: jaburepo::error::RepositoryOperationError) -> Self {
        match value {
            RepositoryOperationError::GetError(req_err) => {
                let error_msg = if let Some(status_code) = req_err.status() {
                    status_code.to_string()
                } else {
                    "No response status code".to_string()
                };
                let resource_name = if let Some(url) = req_err.url() {
                    url.to_string()
                } else {
            ...
            RepositoryOperationError::InvalidArtifactName(e) => Self::Generic(e.to_string()),
            RepositoryOperationError::SerdeXmlParsingError(e) => Self::UnavailableResource {
                resource_name: "A dependency".to_string(), // TODO: Make this more explanatory.
